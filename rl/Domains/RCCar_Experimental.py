...
class RCCarSlideTurn(RCCarBias):
    ...
    def _action_dynamics(self, state, acc, turn):
        if np.linalg.norm(state[0:2] - self.GOAL[:2]) < self.GOAL_RADIUS * 5:
...
class RCCarST_Truncator(RCCarSlideTurn):

    def __init__(self, pnlty_trunc=0.5, **kwargs):
        self._penalty_truncator = -abs(pnlty_trunc)
        super(RCCarST_Truncator, self).__init__(**kwargs)

    def _reward(self, ns, terminal):
        quad_reward = -(np.linalg.norm(ns[:3] - self.GOAL) / ((self.XMAX - self.XMIN)**2+(self.YMAX - self.YMIN) **2) + self.SPEEDMAX ** 2) 
        if quad_reward < self._penalty_truncator:
            quad_reward = -1
        r = self.GOAL_REWARD if terminal else quad_reward
        # Collision to wall => set reward to bad
        if ns[0] == self.XMIN or ns[0] == self.XMAX or ns[1] == self.YMIN or ns[1] == self.YMAX:
...
class RCCarTurn_SWITCH(RCCarBias):
    counter = 0
    def _action_dynamics(self, state, acc, turn):
        assert False, "Don't use"
        self.counter += 1
        if self.counter < 6 * 1e5:
            if self.counter % 1e4 == 0:
                print self.counter
            return self._dynamics_turn(state, acc, turn, bias=RIGHT_TURN)
        else:
            if np.linalg.norm(state[0:2] - self.GOAL[:2]) < self.GOAL_RADIUS * 5:
...
class RCCarSlide_SWITCH(RCCarBias):
    counter = 0
    def _action_dynamics(self, state, acc, turn):
        assert False, "Don't use"
        self.counter += 1
