...
class GridWorldModified(Domain):
    ...
    **ACTIONS:**
    ...
    the destination is not blocked or out of range). -1, 0 is up, 1, 0 is down, 0,-1 is left, 0,1 is right\n
    ...
    **REWARD:**
    ...
    def __init__(self, mapname=os.path.join(default_map_dir, "4x5.txt"),
                 ...
                 random_start=False,
                 episodeCap=None,
                 start_at=None, terrain_augmentation=False):
        self.actions_num = 4

        self.mapname = os.path.join(self.default_map_dir, mapname) if mapname[0] != '/' else mapname

        self.map = np.loadtxt(self.mapname, dtype=np.uint8)
        if self.map.ndim == 1:
            self.map = self.map[np.newaxis, :]
        print start_at
        self.start_state = np.array(start_at) if start_at is not None else np.argwhere(self.map == self.START)[0]

        self.random_start = random_start and start_at is None

        self.ROWS, self.COLS = np.shape(self.map)
        if terrain_augmentation:
            self.statespace_limits = np.array(
                [[0, self.ROWS - 1], [0, self.COLS - 1], [0, 1]])
            self.DimNames = ['Row', 'Col', 'Terrain']
            print "Appending statespace wih binary feature", self.statespace_limits
        else:
            self.statespace_limits = np.array(
        ...
        super(GridWorldModified, self).__init__()
    ...
    def check_valid_state(self, ns):
            return not (ns[0] < 0 or ns[0] >= self.ROWS or
                    ns[1] < 0 or ns[1] >= self.COLS or
                    self.map[ns[0], ns[1]] == self.BLOCKED)
